import { NextResponse } from 'next/server'
import { Prisma } from '@prisma/client'
import { ZodError } from 'zod'

import { HttpError } from '@/lib/server/http/http-error'
import { mapPrismaError } from '@/lib/server/http/prisma-error'

type RouteContext = { params: Promise<Record<string, string>> }
type RouteHandler = (
  request: Request,
  context: RouteContext,
) => Promise<Response>

function fieldErrorsFromZod(error: ZodError): Record<string, string[]> {
  const fieldErrors: Record<string, string[]> = {}
  for (const issue of error.issues) {
    const key = issue.path.join('.') || '_root'
    ;(fieldErrors[key] ??= []).push(issue.message)
  }
  return fieldErrors
}

/**
 * Wraps a route handler so thrown `HttpError`/`ZodError` become consistent JSON
 * error responses and nothing leaks an unhandled 500 stack to the client.
 *
 * Defense-in-depth: services already call `mapPrismaError` themselves, but if any
 * service forgets, this handler catches Prisma errors here too and translates
 * them into the right HttpError before the 500 fallback.
 */
export function handleRoute(handler: RouteHandler): RouteHandler {
  return async (request, context) => {
    try {
      return await handler(request, context)
    } catch (error) {
      // Defense-in-depth: translate any Prisma error a service forgot to map.
      // mapPrismaError throws an HttpError, which the next branch handles.
      if (
        error instanceof Prisma.PrismaClientKnownRequestError ||
        error instanceof Prisma.PrismaClientValidationError
      ) {
        try {
          mapPrismaError(error)
        } catch (translated) {
          if (translated instanceof HttpError) {
            return NextResponse.json(
              {
                code: translated.code,
                message: translated.message,
                fieldErrors: translated.fieldErrors,
              },
              { status: translated.status },
            )
          }
          // mapPrismaError re-threw the original — fall through to the 500 branch.
        }
      }
      if (error instanceof HttpError) {
        return NextResponse.json(
          {
            code: error.code,
            message: error.message,
            fieldErrors: error.fieldErrors,
          },
          { status: error.status },
        )
      }
      if (error instanceof ZodError) {
        return NextResponse.json(
          {
            code: 'VALIDATION_ERROR',
            message: 'Validation failed',
            fieldErrors: fieldErrorsFromZod(error),
          },
          { status: 422 },
        )
      }
      console.error('[api] Unhandled error:', error)
      return NextResponse.json(
        { code: 'INTERNAL_ERROR', message: 'Something went wrong' },
        { status: 500 },
      )
    }
  }
}
