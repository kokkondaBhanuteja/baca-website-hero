...
- [`UseAdminListUrlState`](<app/(admin)/admin/components/admin-list-table/use-admin-list-url-state.claude.md>) — Client hook that turns URL `?page` / `?q` into the controlled props AdminListTable expects. Debounces search, uses `useTransition` for smooth pagination.
...
- [`BlogArticlesTable`](<app/(admin)/admin/components/blog-articles-table/blog-articles-table.claude.md>) — Client wrapper around AdminListTable for /admin/blog-articles: searchable by title/slug/category, 10-per-page, inline Featured badge.
- [`CategoriesTable`](<app/(admin)/admin/components/categories-table/categories-table.claude.md>) — Client wrapper around AdminListTable for /admin/categories: searchable by name/slug, 10-per-page.
...
- [`SiteFooter`](components/layout/site-footer/site-footer.claude.md) — Site-wide footer: flat in-file implementation with 4 i18n-driven nav columns (Products / Company / Reach Us / Certified), legal strip, and oversized BACA wordmark; renders `<ContactStrip />` above the dark band.
...
- [`PasswordInput`](components/ui/password-input/password-input.claude.md) — Reusable password input primitive with show/hide eye toggle. forwardRef'd, RTL-safe, `aria-pressed` toggle button.
...
- [`EnquiryService`](lib/server/services/enquiry-service/enquiry-service.claude.md) — Persists public contact-form submissions and fires a best-effort SMTP notification via the email module.
...
- [`EnquirySchema`](lib/server/validation/enquiry-schema/enquiry-schema.claude.md) — Zod schema for public contact-form submissions and admin status updates.
...
### HTTP helpers (lib/server/http) (5)
...
- [`ParseAdminListQuery`](lib/server/http/parse-admin-list-query/parse-admin-list-query.claude.md) — Parses `?page=&pageSize=&q=` from a Request for the admin paginated list endpoints. Defaults + cap.
...
### Email (lib/server/email) (1)

- [`Email`](lib/server/email/email.claude.md) — SMTP email sender (nodemailer). Best-effort `sendEnquiryNotification` fired from the enquiry service after a DB write. Silently skips when SMTP isn't configured.
...
- [`EnquiryApi`](lib/api-client/endpoints/enquiry-api/enquiry-api.claude.md) — Typed axios wrapper for the single public enquiry submission endpoint.
...
- [`PageAppSiteLocaleContact`](<app/(site)/[locale]/contact/page.claude.md>) — Contact page with PageIntro and three contact-channel cards (Email / Phone / WhatsApp), plus the office address.
...
### Not-found boundaries (app/.../not-found.tsx) (1)

- [`RootNotFound`](app/not-found.claude.md) — Root-level 404. Redirects to home (`/`) — see doc for why a redirect rather than JSX. The localised rich 404 lives at `app/(site)/[locale]/not-found.tsx` (no sibling doc by design); it fires for any unmatched path inside a locale segment and renders the WordmarkSlideshow "404" with full SiteHeader + SiteFooter + WhatsAppFab chrome.

### API route handlers (app/api/.../route.ts) (13)
...
- [`RouteAppApiEnquiry`](app/api/enquiry/route.claude.md) — POST public (contact form, rate-limited). Writes DB row + best-effort SMTP notification. No admin GET.
