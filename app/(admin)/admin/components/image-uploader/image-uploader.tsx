...
/**
 * Accessibility:
 *   - The visible field label is a real `<label htmlFor={inputId}>`, tied to the
 *     hidden file input (so screen readers announce its name).
 *   - The drop-zone surface is a `<label>` referencing the same id — click anywhere
 *     on the drop zone activates the file picker, both for mouse and SR users.
 */
export function ImageUploader({
  ...
  // Guard against setState after unmount during the two-step upload (sign + upload)
  // that can each take several seconds.
  ...
  return (
    <div className="mb-5">
      ...
      {value ? (
        ...
        <label
          htmlFor={inputId}
          className="flex h-40 w-40 cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-line bg-paper text-ink-60 transition-colors hover:border-ink/40 hover:text-ink"
        >
