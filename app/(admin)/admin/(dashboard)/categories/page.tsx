  ...
  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <h1 className="font-heading text-3xl font-light text-ink">Categories</h1>
        <Link
      ...
      {categories.length === 0 ? (
        <p className="text-sm text-ink-60">No categories yet. Create the first one.</p>
      ) : (
            ...
            <tbody>
              {categories.map((category) => (
                <tr key={category.id} className="border-b border-line last:border-0">
                  ...
                  <td className="px-5 py-3 font-mono text-xs text-ink-60">{category.slug}</td>
                  <td className="px-5 py-3 text-ink-60">{category.productCount}</td>
                  <td className="px-5 py-3">
                    <span className={category.isPublished ? 'text-forest' : 'text-ink-60'}>
